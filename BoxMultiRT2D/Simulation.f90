     ...
     &                  ,ke=1
      ...
      real(8),parameter:: x2min=-1.50d0,x2max=1.50d0
...
!         svc(ncsp,i,j,k) = csiso
 ...
 ! for output
     ...
     k=ks
!$omp parallel do collapse(2) private(Pleftc1,Pleftc2,Plefte,Prigtc1,Prigtc2,Prigte,dsvp,dsvm,dsv,leftco,rigtco,nflux)
      do j=js,je
      do i=is,ie+1
         Pleftc1(:) = svc(:,i-2,j,k)
         Pleftc2(:) = svc(:,i-1,j,k)
